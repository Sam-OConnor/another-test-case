...
function startRecursive() {
  ...
  let selectedUsers = users.splice(0, 10)

  return Promise.all(selectedUsers.map(user => processUser(user.id)))
    .then(total => total.reduce((accumulator, currentValue) => accumulator + currentValue))
    .then(result => {
      if (users.length > 0)
        return startRecursive().then(res => result + res)
      else
        return result
    })
}
